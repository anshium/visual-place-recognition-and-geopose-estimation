...
from transformers import AutoImageProcessor, Dinov2Model  # <-- MODIFIED
...
SAVE_DIR = f"/home/skills/ansh/delme/dinov2salad/training_gemini_{DATE_TIME}"
...
# Hyperparameters
MODEL_NAME = "facebook/dinov2-base"  # <-- MODIFIED
IMAGE_SIZE = 224  # DINOv2 uses 224 by default
BATCH_SIZE = 16
LEARNING_RATE = 5e-5
WEIGHT_DECAY = 0.01
NUM_EPOCHS = 50
...
# --- Regression Model using DINOv2 ---
class DINOv2RegressionModel(nn.Module):  # <-- MODIFIED
    ...
    def forward(self, pixel_values):
        ...
        pooled_output = outputs.last_hidden_state.mean(dim=1)  # <-- Use mean-pooled output
...
model = DINOv2RegressionModel(MODEL_NAME, dropout_prob=DROPOUT_PROB).to(device)  # <-- MODIFIED
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    print(f"Epoch {epoch+1} - Avg Train Loss (Scaled): {avg_train_loss:.4f}")
    ...
    print(f"  MSE: {val_mse:.4f}, RMSE: {val_rmse:.4f}, MAE: {val_mae:.4f}")
    ...
    for i in range(min(5, len(all_preds_original))):
        ...
        print(f"  Pred: [{pred[0]:.4f}, {pred[1]:.4f}], True: [{true[0]:.4f}, {true[1]:.4f}], Error: [{error[0]:.4f}, {error[1]:.4f}]")
    ...
    if val_mse < best_val_mse:
        print(f"Validation MSE improved: {best_val_mse:.4f} → {val_mse:.4f}. Saving best model...")
...
print(f"Best model saved to {BEST_MODEL_PATH} with Val MSE: {best_val_mse:.4f}")
